...
plot_elev_profile_plus <- function(track,summary,savefn,title="Ride",palette="plasma",
  ...
  return(
    plot_profile(track,summary,savefn,title,palette,
                  ...
                  cadCont=cad.cont,cadContLow=cad.cont.low,cadContHigh=cad.cont.high,
                  ...
                  cadSmoothBW=cad.smooth.bw.meters,cadSmoothNN=cad.smooth.nn,
...
plot_profile <- function(track,summary,savefn,title="Ride",palette="plasma",
                                  ...
                                  hrDistance=FALSE,cadDistance=FALSE,
  ...
  heightBelow <- heightWith(hrDistance,cadDistance,
                          hrTime,cadTime,showTime,totalCall=TRUE)
  grlist <- drawProfile(distancevec=distance,
                        ...
                        elevationShape,
                        imperial)
  if (showSummary & !missing(summary)) grlist <- drawSummary(grlist,summary,title)
...
#  this is key to having text being kept at an appropriate size.  trial-and-error tuning
  ...
  plot.height <- plot.width*((ymax-ymin+400)*vertmult/5280)/((ngraphpoints+800)*distPerPoint)
